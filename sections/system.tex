...
routing is \textbf{$+0.012$ area} ($\sim$2 points at the balanced budget).
The gate's case over type routing therefore rests not on this
in-distribution margin but on transfer: the type shortcut is exactly what
...
test set and frozen expert answers). Under LOPO it collapses entirely
...
the pool a router most needs to catch. Query-feature routing is the type
shortcut and nothing else; the comparison against preference-trained
routers on the LLMRouterBench protocol \citep{llmrouterbench2026} remains
