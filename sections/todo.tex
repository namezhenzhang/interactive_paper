...
\item \todonote{2026-08-20 noise audit (RESULTS \S 8ad, all \$0) --- THREE
  WRITING CHANGES ARE MANDATORY BEFORE SUBMISSION. (a) fig:live and
  fig:pareto were built from probe-\emph{v1} aggregates
  (\texttt{live\_dualview.json} mtime 2026-07-30, sourced from
  \texttt{gated\_traces\_v2.parquet}) while the text quoted v3 ---
  rebuilt from \texttt{frozen\_v3\_traces} by
  \texttt{figures/live\_v3\_figures.py} (v1 archived \texttt{*\_v1});
  every arm number in \S\ref{sec:live} must be re-read off the new
  json (heard .383/.483/.554/.596 at 0/16/35/61\%, channel cost
  $-.175$, P50 2.02/2.61/3.53/4.44\,s, P99 32.7\,s at balanced).
  (b) the sllama headline must move from ``the small model BEATS the
  expert on the easy half'' to ``MATCHES it at half the cost'':
  .976 vs .968 is McNemar $p=1.00$; what is significant is the probe's
  split ($z=6.46$) and the hard-half lift (.696$\to$.888,
  $p<.0001$); the .948-vs-.928 headline itself is $p=.125$.
  (c) report the replication-noise floor (same query kept local in two
  arms flips 2.3--18.8\% of the time; paired SE .009--.028) and STOP
  narrating sub-3-point live deltas --- all 18 v2$\to$v3 arm deltas are
  n.s.; the v3 evidence is the offline AUC, not the live curve.
  Add \texttt{noise\_audit} as a methods figure.}
\item \todonote{deployment artifact: \texttt{data/gate\_v3\_thresholds\_
  corrected.json} (label-free quantiles). Only the frozen pool drifts
  (fires .613 at the 50\% budget); correcting it removes 11.3\% of the
  expert calls at equal accuracy (+.004, n.s.). The externals already
  hit 15/30/50.}
\item \todonote{next spend, ranked by the audit: (1) audio-direct-to-expert
  --- the gold/heard gap on the frozen pool is now $-.175$, an order
  above the noise floor, and it is the only lever whose effect a single
  live sweep can resolve; (2) prefer variance-reduced readouts (AUC,
  matched-rate precision, the \texttt{rate\_curves.json}
  reconstruction) over arm accuracy; (3) only then buy larger $n$.}
\item \todonote{2026-08-20 cloud-ASR uplink (RESULTS \S 8ae) --- the
  channel lever finally has a significant result: on the 147 escalated
  frozen ids, sending the wav to \texttt{gpt-transcribe} and giving
  \emph{that} text to gpt-5.5 scores .694 vs .585 for the deployed
  self-transcript (paired McNemar $p=.007$), recovering 38\% of the
  gap to the .871 gold ceiling. This supersedes the ``Whisper ears
  $+4$pp only'' write-off (8d used open-weights whisper-large-v3).
  hard-math recovers NOTHING (.294 both ways, gold 1.000) --- spoken
  LaTeX is lossy at the source, so \S\ref{sec:scope} should state the
  input-side limit separately from the uplink one. Still to measure
  before this can be claimed as a system result: the ASR call's
  latency on the critical path, and whether $+.109$ survives on the
  external pools.}
\item \todonote{demo app (RESULTS \S 8af):
  \url{https://rhe9527--gate-demo-web.modal.run/62dc5cd9} --- probe
  ON/OFF over the measured sessions plus a real H100 turn on a typed
  question. Useful as a talk/demo artifact; consider a figure or
  appendix screenshot of the end-of-turn read (54 ms, P(fail) .807 vs
  the .680 balanced threshold on ``what is NVDA trading at'').}
\end{itemize}
